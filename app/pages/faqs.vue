    ...
    <main>
        ...
        <section class="bg-emerald-syariah py-16">
            <div class="container mx-auto px-4 md:px-6 text-center">
                ...
                <p class="text-emerald-100/80 max-w-2xl mx-auto mb-10 text-sm md:text-base leading-relaxed">
                    Temukan jawaban atas semua pertanyaan Anda mengenai proses pembelian properti syariah, skema
                    pembayaran, dan legalitas akad di SyariahPro.
                </p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <aside class="hidden lg:block lg:col-span-3">
                    <div class="sticky top-28 space-y-2">
                        ...
                        <a v-for="category in faqCategories" :key="category.id" :href="`#${category.slug}`"
                            class="flex items-center gap-3 p-3 rounded-xl text-sm font-semibold transition-all" :class="category.id === faqCategories[0].id
                        ...
                        <div
                            ...
                            <p class="text-xs text-emerald-100/60 mb-6">
                                Tim marketing kami siap membantu menjawab pertanyaan detail Anda.
                            </p>
                            <a href="#"
                ...
                <div class="col-span-1 lg:col-span-9 space-y-16">
                    <section v-for="category in faqCategories" :key="category.id" :id="category.slug">
                        ...
                        <div class="space-y-4">
                            <article v-for="item in category.items" :key="item.id"
                                ...
                                :class="isActive(item.id)
                                    ...
                                    : 'border-gray-100 hover:border-emerald-syariah'" @click="toggleFaq(item.id)">
                                <div class="flex items-center justify-between gap-4">
                                    <h3 class="font-bold text-lg"
                                        :class="isActive(item.id) ? 'text-emerald-syariah' : 'text-gray-800'">
                                        {{ item.question }}
                                    </h3>
                                    <Icon name="lucide:chevron-down" class="text-xl shrink-0 transition-transform"
                                        :class="isActive(item.id) ? 'text-emerald-syariah rotate-180' : 'text-gray-400'" />
                                </div>
...
const activeFaqId = ref<string | null>(faqCategories[0]?.items[0]?.id ?? null);

const isActive = (id: string) => activeFaqId.value === id;
...
useHead({
    title: 'FAQ Properti Syariah | SyariahPro'
});
</script>
